...
fun MoreScreen(padding: PaddingValues, current: AppTheme, onTheme: (AppTheme) -> Unit) {
    ...
    Column(Modifier.fillMaxSize().padding(padding).padding(20.dp)) {
        ...
        Text("所有设置均为本地演示入口", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
        Spacer(Modifier.height(20.dp))
        Section("下载", listOf("下载偏好" to "尚未接入下载引擎", "保存位置" to "尚未申请文件权限"))
        Section("隐私", listOf("Cookie 管理" to "未启用", "本地数据" to "没有创建数据库"))
        Section("应用", listOf("外观" to current.label, "关于与支持" to "TCPGYT · 本地优先")) { if (it == "外观") sheet = true }
    }
    if (sheet) ThemeSheet(current, { onTheme(it); sheet = false }, { sheet = false })
}

@Composable
private fun Section(title: String, items: List<Pair<String, String>>, click: (String) -> Unit = {}) {
    Text(title, style = MaterialTheme.typography.labelLarge, color = MaterialTheme.colorScheme.primary, modifier = Modifier.padding(top = 8.dp, bottom = 6.dp))
    items.forEach { Setting(it.first, it.second) { click(it.first) } }
}
